...
function hasAZero(num) {
    return num.toString().split('').some(function(val){ //had to look at solution, look up split
        return val === '0';
    })
}
...
duplicate values (more than one element in the array that has the same value as another). If there are 
any duplicates, the function should return false.
...
function hasNoDuplicates(arr) {
    return arr.every(function(val){
        return arr.indexOf(val) === arr.lastIndexOf(val); //had to look at solution, look up lastIndexOf
    })
}
...
function hasCertainKey(arr, key) {
    return arr.every(function(obj){
        return key in obj;      //idk what this is. 
    })
}
